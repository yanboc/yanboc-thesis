...
设$d \in \mathbb{N}^{+}$为向量维度，$\x_1, \x_2 \in \R^d$为从单位球面$\{\x \in \R^d : \|\x\| = 1\}$上独立均匀随机抽取的向量。
对于任意常数$c > 0$，向量内积的概率上界满足：
\begin{equation}\label{eq-prop-high-dim-ortho}
\P\left[\left|\x_1^{\top} \x_2\right| \geq \frac{c}{\sqrt{d-1}}\right] \leq \frac{2}{c} e^{-\frac{c^2}{2}}.
\end{equation}
特别地，当$d \to \infty$时，对于任意$\epsilon > 0$，有
\begin{equation}
\P\left[\left|\x_1^{\top} \x_2\right| \geq \epsilon\right] \to 0.
\end{equation}
