...
此处AT指通过特定训练范式增强神经网络对抗鲁棒性的方法\cite{DBLP:journals/corr/GoodfellowSS14}, 具体定义可参阅\S\ref{sec-at-preliminaries}.
...
\begin{figure}[H]
  ...
  我们绘制了三种对抗训练方法 (包括\texttt{PGD-AT}, \texttt{TRADES}和\texttt{MART}) 在不同扰动半径$\epsilon \in \{0, 2/255, 4/255, 6/255, 8/255\}$下的单纯形范数$P_c$ (参见等式\ref{eq-def-P-eps}) 观测值.
  此处$\epsilon = 0$表示干净训练. 单纯形范数是使用训练数据计算的. 
  底部：三分类任务中单纯形压缩现象的可视化. 我们观察到几何尺寸 (即单纯形 ETF 结构在对抗训练下的单纯形范数$P_c$) 的减小；且随着扰动半径的增加, 单纯形范数减小 (此处A, B和C分别代表扰动半径的增加) . 
...
\begin{definition}[特征塌缩]\label{def-neural-collapse}
    ...
    \begin{itemize}
        ...
        在TPT阶段, 分类器会趋近于选择与训练类均值最近 (在标准欧氏距离下) 的类别, 即：
            ...
            \end{equation}
        其中 $\tilde{\boldsymbol{\mu}}_{c}=\left(\boldsymbol{\mu}_{c}-\boldsymbol{\mu}_{G}\right) /\left\|\boldsymbol{\mu}_{c}-\boldsymbol{\mu}_{G}\right\|_{2}$ 为重新缩放地类均值 (renormalized class mean). \myqed
    \end{itemize}
\end{definition}
...
\begin{figure}[htbp]
  ...
  \caption{
    本图展示了单纯形范数$\boldsymbol{P}_{\epsilon}$的观测值随扰动半径的变化趋势.
    我们在\texttt{CIFAR-10}数据集上使用\texttt{PGD-AT}训练\texttt{ResNet-18}. 
    我们选择$T_{a}=\{ 10,20,30\}$ (参考定义\ref{eq-def-GD-AT}) 和范数$p=\{L_\infty , L_2\}$. 
    结果来自对抗样本.
    可以发现, 随着扰动半径变大, 各个实验组的单纯形范数展现出显著的下降趋势.
    }
...
\begin{figure}[htbp]
  ...
  \label{fig-sample-effect}
\end{figure}

\section{关于单纯形压缩现象的理论解释}\label{sec-theoretical-explanation}

在本节中, 我们通过分析干净训练和对抗训练过程, 理论解释单纯形压缩现象. 
我们指定数据模型, 网络结构和训练范式在\S\ref{sec-nc-preliminaries}中. 
假设总结在\S\ref{subsec-nc-assumptions}中. 
...
如\S\ref{sec-nc-preliminaries}所述, 标准和对抗NC都发生在TPT期间.
...
\end{equation}
其中$\left( \boldsymbol{e}_1, \boldsymbol{e}_2, \cdots, \boldsymbol{e}_K  \right) \in \R^{\dpr \times K}$是一个二元半正交矩阵, 使得对于任意$k \in [K]$, 只有$\boldsymbol{e}_k$的第$k$个元素取$1$, 而其他元素取$0$. \myqed
\end{assumption}
